...
const validateForm = (values, sameAsTrack, sameAsMailing) => {
  if (sameAsTrack || sameAsMailing) {
    return true;
  }
  ...
  return (
    <>
      ...
      <Button
        ...
        small={true}
        buttonColor={
          validateForm(signupValues, sameAsTrack, sameAsMailing)
            ? 768 > width
              ? '#fa4616'
              : '#00001d'
            : '#6c6c79'
        ...
        textColor={'#fff'}
        onClick={async () => {
          if (!validateForm(signupValues, sameAsTrack, sameAsMailing)) {
            return;
          }

          const account = storage.get('account');

          const data = sameAsTrack
          ...
          if (result?.data?.addAddress) {
            storage.set('account', {
              ...account,
              mailingAddress: signupValues,
            });
            setStep('AccountSuccess');
          } else {
